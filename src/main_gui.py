...
from tracker import ImageTracker
from search import ImageSearcher
from drive_uploader import DriveUploader
from bib.preprocess import PreprocessConfig, preprocess_image
...
class ImageCard(QFrame):
    ...
    def __init__(self, image_data: Dict, thumbnail_path: Path, parent=None):
        super().__init__(parent)
        ...
        self.thumbnail_label.mousePressEvent = self.show_full_image
...
class UploadWorker(QThread):
    ...
    def __init__(
        ...
        self.preprocess_out_dir = preprocess_out_dir
    ...
    def run(self):
        ...
        try:
            ...
            for i, (url_hash, file_path) in enumerate(self.images):
                # 1) Preprocess (plan step 2.2) BEFORE uploading
                try:
                    entry = self.tracker.get_image_by_url_hash(url_hash) or {}
                    md = entry.get("metadata") or {}
                    processed_path = None

                    existing = md.get("bib_preprocess_final")
                    if existing:
                        p = Path(existing)
                        if p.exists():
                            processed_path = p
                    ...
                    if processed_path is None:
                        self.progress.emit(f"Preprocessing {file_path.name}", i + 1, total)
                        out_dir = Path(self.preprocess_out_dir) / url_hash
                        stages = preprocess_image(
                            file_path,
                            self.preprocess_cfg,
                            save_debug_to=out_dir,
                            prefix=url_hash,
                        )
                        processed_path = stages["final"]
                        self.tracker.update_metadata(
                            url_hash, {"bib_preprocess_final": str(processed_path)}
                        )

                except Exception as e:
                    self.tracker.update_status(url_hash, "failed")
                    failed += 1
                    self.error.emit(f"Preprocess failed for {file_path}: {e}")
                    continue

                # 2) Upload the PREPROCESSED file
                self.progress.emit(f"Uploading {processed_path.name}", i + 1, total)

                file_id = self.uploader.upload_file(processed_path)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, config_path: str = "secrets/config.json"):
        ...
        # Bib preprocessing configuration (used automatically before upload)
        bib_cfg = (self.config.get("bib_extraction") or {})
        preprocess_cfg_dict = (bib_cfg.get("preprocess") or {})
        # Only accept known keys (so config.json can contain extra fields without crashing)
        allowed = set(PreprocessConfig.__dataclass_fields__.keys())
        safe_kwargs = {k: v for k, v in preprocess_cfg_dict.items() if k in allowed}
        self.bib_preprocess_cfg = PreprocessConfig(**safe_kwargs)
        self.bib_preprocess_out_dir = Path(bib_cfg.get("output_dir", "processed_bib"))
    ...
    def download_full_image(self, result: Dict, card: ImageCard):
    ...
    def upload_selected(self):
        ...
        self.upload_worker = UploadWorker(
            ...
            self.tracker,
            selected,
            self.bib_preprocess_cfg,
            self.bib_preprocess_out_dir,
        )
    ...
    def load_previous_session(self):
        ...
        for img_data in pending_images:
            ...
            display_path = thumbnail_path if (thumbnail_path and thumbnail_path.exists()) else local_path
